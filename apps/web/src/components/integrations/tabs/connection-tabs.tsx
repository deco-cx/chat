...
function ParametersViewer({ tool }: Pick<ToolProps, "tool">) {
  function getParameters(schema: Record<string, unknown>) {
...
function ToolsInspector({
  ...
  useEffect(() => {
    ...
    globalThis.addEventListener("refresh-tools", handleRefresh);
    return () => globalThis.removeEventListener("refresh-tools", handleRefresh);
  }, []);
  ...
  const tools = useTools(
    ...
    ignoreCache.current || refreshTrigger > 0,
  );
