...
import { type AppContext, serializeError } from "./context";
import z from "zod";
...
export const withMCPAuthorization =
  ...
  async (
    ...
    try {
      ...
      console.error (`withMCPAuthorization error: user id ${ctx.user?.id} failed to access ${integrationId} resource ${req.params.name} at workspace ${ctx.workspace?.value}`);
      if (!DRY_RUN_AUTH) {
        return {
          isError: true,
          content: [{ type: "text", text: serializeError(error) }],
